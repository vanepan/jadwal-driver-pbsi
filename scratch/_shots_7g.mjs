...
const VIEWPORTS = {
  ...
  mobile402: { width: 402, height: 874, isMobile: true },
  ...
  return {
    ...
    models: {
      exec: { driverKpis: { activeVehicles: 5, activeDrivers: 4 }, score: { value: 78, level: 'good', label: 'Baik' }, scoreBreakdown: { components: [
        { key: 'vehicleUtil', label: 'Armada', score: 72 },
        { key: 'driverOps', label: 'Driver', score: 88 },
        { key: 'request', label: 'Permintaan', score: 95 },
        { key: 'engineering', label: 'Teknik', score: 61 },
        { key: 'pettyCash', label: 'Petty Cash', score: 100 },
      ] } },
...
async function shot(vp, theme) {
  ...
  const overflow = await page.evaluate(() => {
    ...
    return { scrollWidth: doc.scrollWidth, clientWidth: doc.clientWidth, overflows: doc.scrollWidth > doc.clientWidth + 1 };
  });
  const name = `phase7g-${vp}-${theme}.png`;
  await page.screenshot({ path: path.join(SHOTS, name), fullPage: true });
  console.log('saved', name, overflow.overflows ? `OVERFLOW! scrollWidth=${overflow.scrollWidth} clientWidth=${overflow.clientWidth}` : 'no-overflow');
}

const only = process.argv[2];
const vps = only ? [only] : Object.keys(VIEWPORTS);
for (const vp of vps) {
  await shot(vp, 'light');
  await shot(vp, 'dark');
}
