...
// V1.31.1 "Agenda, Kalender & To-Do" — the spec's own worked example.
function cal(overrides = {}) {
  return {
    id: 'c' + Math.random(), title: 'Evan - Sirnas C Piala Raja', scope: 'sarpras_shared',
    organizerUsername: 'evan', status: 'scheduled',
    startDate: '2026-09-08', endDate: '2026-09-12', allDay: true,
    startAt: new Date('2026-09-08T00:00:00+07:00').getTime(), endAt: new Date('2026-09-12T23:59:00+07:00').getTime(),
    participants: { evan: { isPic: true, status: 'accepted' }, kabid: { isPic: false, status: 'invited' } },
    ...overrides,
  };
}
...
const VARIANTS = [
  { name: 'week-combined', input: { events: [evt(), evt({ id: 'e2', title: 'Kunjungan Kerja Bidang Sarpras', type: 'kunjungan', date: '2026-09-11', location: 'Kantor Pusat' })], tasks: [task(), task({ id: 't2', title: 'Task Selesai', status: 'done', dueDate: '2026-09-08' })], calendarItems: [cal(), cal({ id: 'c2', title: 'Rapat Dibatalkan', startDate: '2026-09-07', endDate: '2026-09-07', status: 'cancelled' })], range: RANGE_WEEK, filters: { mode: 'semua' }, directory: DIRECTORY, now: NOW } },
  { name: 'kalender-only', input: { events: [], tasks: [], calendarItems: [cal()], range: RANGE_WEEK, filters: { mode: 'kalender' }, directory: DIRECTORY, now: NOW } },
...
try {
  ...
  console.log('\n=== [D — V1.31.1 Calendar section renders through the FULL real pdfmake pipeline] ===');
  await checkAsync('the week-combined report\'s rendered PDF contains the multi-day Calendar item (Sirnas), a Kalender section, and its status vocabulary NEVER includes "overdue" (the fixture ALSO has a genuinely overdue TASK, which legitimately renders "Terlewat" elsewhere in the SAME document — this checks the Calendar item specifically, not the whole document)', async () => {
    const result = await page.evaluate((input) => window.__buildAndGenerate(input), VARIANTS[0].input);
    const defJson = JSON.stringify(result.definition);
    if (!defJson.includes('Sirnas C Piala Raja')) throw new Error('Calendar item title missing from the rendered PDF');
    if (!defJson.includes('Kalender')) throw new Error('no "Kalender" section title in the rendered PDF');
    if (!defJson.includes('Dibatalkan')) throw new Error('the cancelled calendar item\'s "Dibatalkan" status label is missing');
    const calendarStatuses = result.vm.calendarItems.map((c) => c.status);
    if (calendarStatuses.some((s) => s === 'overdue')) throw new Error(`a Calendar item's status must NEVER be 'overdue' — got ${JSON.stringify(calendarStatuses)}`);
    if (typeof result.vm.summary.totalCalendarItems !== 'number') throw new Error('summary.totalCalendarItems missing from the view-model');
  });
