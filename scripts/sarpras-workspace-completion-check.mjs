...
} from '../src/ui/sarpras-intelligence-center.js';
import { mountArchiveCenter, closeArchiveCenter } from '../src/ui/archive-center.js';
import { mountKnowledgeCenter, closeKnowledgeCenter } from '../src/ui/knowledge-center.js';
import { mountLearningDashboard, closeLearningDashboard } from '../src/ui/learning-dashboard.js';
import { mountSarprasSettings, closeSarprasSettings } from '../src/ui/sarpras-settings.js';
...
const norCenterSrc = fs.readFileSync(path.join(ROOT, 'src/ui/nor-center.js'), 'utf8');
...
const outerShellSrc = fs.readFileSync(path.join(ROOT, 'src/ui/sarpras-intelligence-center.js'), 'utf8');
...
for (const file of ['src/ui/archive-center.js', 'src/ui/knowledge-center.js', 'src/ui/learning-dashboard.js', 'src/ui/dataset-import-center.js']) {
  const src = fs.readFileSync(path.join(ROOT, file), 'utf8');
  check(`${file} contains no literal "Coming Soon"`, !src.includes('Coming Soon') && !src.includes('segera hadir'));
}
{
  const norCenterFullSrc = fs.readFileSync(path.join(ROOT, 'src/ui/nor-center.js'), 'utf8');
...
const archiveSrc = fs.readFileSync(path.join(ROOT, 'src/ui/archive-center.js'), 'utf8');
...
const knowledgeSrc = fs.readFileSync(path.join(ROOT, 'src/ui/knowledge-center.js'), 'utf8');
...
const learningSrc = fs.readFileSync(path.join(ROOT, 'src/ui/learning-dashboard.js'), 'utf8');
...
const datasetImportSrc = fs.readFileSync(path.join(ROOT, 'src/ui/dataset-import-center.js'), 'utf8');
...
const norCenterSrc2 = fs.readFileSync(path.join(ROOT, 'src/ui/nor-center.js'), 'utf8');
...
check('sarpras-intelligence-center.js calls all three V2.1.2 persistence sync functions on mount', (() => {
  const src = fs.readFileSync(path.join(ROOT, 'src/ui/sarpras-intelligence-center.js'), 'utf8');
...
const metadataInferenceSrc = fs.readFileSync(path.join(ROOT, 'src/knowledge/datasets/import-session/metadata-inference-engine.js'), 'utf8');
...
for (const file of workspaceFiles) {
  const src = fs.readFileSync(path.join(ROOT, `src/ui/${file}`), 'utf8');
...
check('the pipeline scheduler exists — the ONE driver of the Import Session lifecycle', fs.existsSync(path.join(ROOT, 'src/knowledge/datasets/import-session/pipeline-scheduler.js')));
...
{
  const schedulerSrc = fs.readFileSync(path.join(ROOT, 'src/knowledge/datasets/import-session/pipeline-scheduler.js'), 'utf8');
