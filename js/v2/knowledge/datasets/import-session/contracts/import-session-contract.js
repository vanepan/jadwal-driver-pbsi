 ...
 * @property {Object|null} factsProvenance  - V2, Part A1/A2/B1: `{source: 'human'|'auto-extraction'|'evidence-resolution', contentParserVersion: number|null, metadataParserVersion: number|null, confidencePerField: Object|null, recordedAt: string}` — who/what last wrote `manualEntryFacts`, and with what parser version/confidence. Null until the first write. Lets Part A2's re-analysis sweep know a field was human-confirmed (never silently overwritten) vs. machine-derived (safe to replace with strictly-higher-confidence evidence later). `'evidence-resolution'` (V2, Part B1) means at least one field came from cross-document consensus (content-fact-consensus-engine.js) rather than this document's own text alone — still never `'human'`, so the same "never silently overwritten" protection only ever applies to a field a person actually typed
 ...
 * @property {Object|null} consensusSuggestion - V2, Part B1 (Evidence-First Ingestion): `{senderOrigin: {value, supportCount, totalCount, agreement, confidence, eligible, rationale}, computedAt}` — content-fact-consensus-engine.js's result for whichever fields extraction alone could not resolve on THIS document, ALWAYS recorded when attempted (even when inconclusive), same "always record the attempt, never let it silently satisfy the gate on its own" discipline as `extractionSuggestion`. Only ever computed for fields where cross-document consensus is a legitimate evidence source (see that engine's own header for why documentNumber/value are excluded on principle, not merely unimplemented)
...
export function normalizeImportSessionRecord(r) {
  ...
  return {
    ...
    consensusSuggestion: r.consensusSuggestion ?? null,
