/* ============================================================
   REPOSITORY-REGISTRY.JS — Knowledge Repository Foundation (V2, Phase 5)

   PURPOSE: the single process-wide directory of repository backends,
   mirroring js/prediction/prediction-provider.js's registry. Bootstraps
   NullRepository and the shared MemoryRepository instance, with
   NullRepository active by default.

   RESPONSIBILITY: register/get/list repositories and track which one is
   active.

   DEPENDENCIES: contracts/repository-contract.js,
   implementations/null-repository.js, implementations/memory-repository.js.

   NON-GOALS: does not decide which backend a real caller SHOULD use — that
   is a Phase 6+ (Services) or deployment-time decision. Defaulting to
   'null' (not 'memory') is deliberate: MemoryRepository is a real
   implementation, but silently defaulting to non-durable storage would
   let a caller believe data is being kept when it is not, the first time
   the process restarts.

   FUTURE EVOLUTION: a future Firebase-backed repository registers here
   alongside Null and Memory; a deployment can call
   `setActiveRepository('firebase')` with zero changes to any caller.
   ============================================================ */
